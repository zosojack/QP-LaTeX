...
\end{cases}
\hspace{1.5ex} \implies \hspace{1.5ex} 
\bigl[\hat{x}_1,\hat{x}_2\bigr]=\dfrac{i}{2}.
\end{equation}

\callout{Commutator between quadrature operators}{%
\begin{proof}
    To perform the computation, it is just needed to remember the commutation relation between the ladder operators, $[\hat{a},\hat{a}^\dagger] = 1$ (Eqn.~\ref{eqn:11-commutation-annihilation-creation-operators}):

\begin{equation}
\begin{split}
\bigl[\hat{x}_1,\hat{x}_2\bigr] &= \frac{1}{4i} 
\left[
    \hat{a} + \hat{a}^\dagger, \hat{a} - \hat{a}^\dagger
\right] = \\[0.5ex]
&= \frac{1}{4i} \left[\hat{a}, -\hat{a}^\dagger\right] + 
\frac{1}{4i} \left[\hat{a}^\dagger, \hat{a}\right] = \\[0.5ex]
&= \frac{1}{4i} \left(-1 - 1\right) = \frac{i}{2}
\end{split}
\end{equation}
\end{proof}
}

By expanding the exponentials via Euler's formula, \hl{the time-dependent electric field can be elegantly expressed as}:
...
\callout{Derivation of Equation~\eqref{eqn:11-electric-field-quadrature-operators}}{%
\begin{proof}
Starting from the Eqn.~\ref{eqn:11-single-mode-field-time}, using the Euler's relations in \eqref{eqn:3-euler-formula} and finally recognizing the definitions of the quadrature operators in \eqref{eqn:11-quadrature-operators}, we can rewrite the electric field as:
\begin{equation}
\begin{split}
    \hat{E}_x(z,t) &= 
    A_0\left(
        \hat{a}e^{-i\omega t}+\hat{a}^\dagger e^{i\omega t}
    \right) \sin(kz) = \\[1ex]
    &= A_0\left[
        \hat{a}\left(\cos(\omega t) - i\sin(\omega t)\right) +
        \hat{a}^\dagger\left(\cos(\omega t) + i\sin(\omega t)\right)
    \right] \sin(kz) = \\[1ex]
    &= A_0\left[
        \left(\hat{a} + \hat{a}^\dagger\right)\cos(\omega t) - \frac{1}{i}
        \left(\hat{a}^\dagger - \hat{a}\right)i\sin(\omega t)
    \right] \sin(kz)  \equiv \\[1ex]
    &\equiv 2A_0\sin(kz) \left[
        \hat{x}_1 \cos(\omega t) + \hat{x}_2 \sin(\omega t)
    \right]
\end{split}
\end{equation}
\end{proof}
...
\begin{align}
    \Delta \hat{x}_1 \, \Delta \hat{x}_2 &= \sqrt{\frac{m\omega }{2\hbar}}\, \Delta\hat{q} \, \sqrt{\frac{1}{2m\hbar \omega}}\, \Delta \hat{p} = \nonumber \\[0.5ex]
...
The expectation values of the quadrature operators on the \textbf{number state} are:
\begin{equation}\label{eqn:11-number-state-quadratures-expectation-values}
    \braket{n|\hat{x}_1|n} = \braket{n|\hat{x}_2|n} =0
\end{equation}

\vspace{0.25ex}

\callout{Proof for $\hat{x}_1$}{%
\begin{proof}
To perform the calculation, it is just needed to remember the definition of the quadrature operator $\hat{x}_1$ in Eqn.~\ref{eqn:11-quadrature-operators}:
\begin{equation}
\begin{split}
    \bra{n}\hat{x}_1\ket{n} &=
    \bra{n} \frac12 \left(\hat{a}+\hat{a}^\dagger\right) \ket{n} = \\[0.5ex]
    &= \frac12 \left[
        \bra{n}\hat{a}\ket{n} + \bra{n}\hat{a}^\dagger\ket{n}
    \right] = \\[0.5ex]
    &= \frac12 \left[
...
while the uncertainties are:
\begin{equation}\label{eqn:11-number-state-quadratures-uncertainties}
\begin{split}
    \bra{n} \hat{x}_1^2 \ket{n} &= \frac{1}{4}\, \bra{n} (\hat{a}^\dagger)^2 + \hat{a}^2 + \hat{a}^\dagger\hat{a}+\hat{a}\hat{a}^\dagger \ket{n} = \\[0.5ex]
    &=\frac{1}{4}\, (0+0+\bra{n} 1+2\hat{a}^\dagger\hat{a} \ket{n}) = \\[0.5ex]
    & = \frac{1}{4}\, (1+2n) = \bra{n} \hat{x}_2^2 \ket{n} 
\end{split}
\end{equation}
