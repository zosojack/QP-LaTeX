...
\begin{equation*}
\begin{cases}
    \vec{E}_1(z,t) = (E_{0x},0,0)\cos(kz-\omega t+\phi_1)\\
    \vec{E}_2(z,t) = (0,E_{0y},0)\cos(kz-\omega t+\phi_2)
\end{cases}
\end{equation*}
...
\Rightarrow
I_{\text{transmitted}} \propto E_{0x}^2
\]
...
Now suppose that, instead of a laser, we use a discharge lamp as the light source. In this case, the atoms in the lamp emit independently, so the outgoing light is an incoherent mixture of many polarization states with the same frequency. As a result, the light is not polarized overall. If such light passes through a polarizer, the transmitted beam becomes linearly polarized along the transmission axis, while the orthogonal component is removed.
...
This property makes single-photon beam splitters a useful physical source of randomness. In practice, optical quantum random number generators are widely used because they exploit intrinsic quantum randomness rather than algorithmic pseudo-randomness.
...
\begin{enumerate}
    ...
    \item \textbf{Lotteries and gambling}, where fairness requires unbiased random outcomes.
