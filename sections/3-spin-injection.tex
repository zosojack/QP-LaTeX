...
\[
M = \int_0^{+\infty} \d{r} \, \int_0^{\pi} \d\theta \, \int_0^{2\pi} \d\phi \, \psi_f^*\,  \hat{H} \, \psi_i\, r^2 \sin^2(\theta)
\]
...
\[
M_{\sigma^{\pm}} \propto \int_0^{2\pi} d\phi\,  Y_{\ell',m'}^*(\theta,\phi) \, e^{\pm i \phi} \, Y_{\ell,m}(\theta,\phi) \propto \int_0^{2\pi} d\phi\,  e^{-im'\phi} \, e^{\pm i \phi} \, e^{im\phi} \ne 0
\]
...
where $I^\pm$ denotes the intensity of the luminescence component with helicity $\sigma^\pm$. \hl{If now we consider the case of a $\sigma^+$ circularly polarized excitation, we have a net spin-down population, so that $n_{\downarrow} = 3 n_{\uparrow}$}, and the resulting circular polarization of the emitted light is:
\begin{equation}\label{eqn:3-circular-polarization-photoluminescence-2}
\rho_c = 
\frac{(1 n_{\uparrow} + 3\cdot3 n_{\uparrow}) -
(3 n_{\uparrow} + 1\cdot3 n_{\uparrow})}%
{(1 n_{\uparrow} + 3\cdot3 n_{\uparrow}) +
(3 n_{\uparrow} + 1\cdot3 n_{\uparrow})} =
\frac{10-6}{10+6} = \frac{4}{16} = 25\%.
\end{equation}
...
\ket{\uparrow_z(t)} =
\ket{\uparrow_z}\cos\left(\frac{\omega_L t}{2}\right) + i\ket{\downarrow_z}\sin\left(\frac{\omega_L t}{2}\right)
\end{equation}
