...
\question{How do we count photons?}{%
\begin{itemize}
...
\begin{enumerate}
    \item \hl{A short temporal width of the output pulse}, so that individual events can be clearly separated from one another and from spurious pulses. Shorter pulses allow higher count rates, typically on the order of $10-20$ \unit{\nano\second}.
    \item \hl{A high quantum efficiency $\eta$ of the photocathode and a low anode dark current}. This is essential to maximize the signal-to-noise ratio, since a high number of primary photoelectrons must be generated while thermally induced electrons must be minimized.
    \item \hl{An appropriate pulse-height distribution}. The pulse heights corresponding to true signal events and to dark counts may overlap only partially, and in general the average amplitude of signal pulses is expected to be higher than that of dark pulses. 
    \begin{figure}[H]
        \centering
    ...
    \item \hl{A careful choice of the photomultiplier voltage and of the discrimination threshold}. If the count rate is measured as a function of the supply voltage, the signal typically increases faster than the noise at sufficiently high voltages. This happens because the discriminator suppresses a large fraction of the dark counts; only after the threshold voltage $V_0$ is reached does the signal-to-noise ratio saturate and approach a plateau.
    \begin{figure}[H]
        \centering
...
\subsection{Time-correlated photoluminescence (photon counting)}
Time-correlated photon counting is a technique based on the statistics of light emission following excitation by a short light pulse. \hl{After excitation, the luminescence signal builds up and then decays in time}. The detector records the arrival times of single photons, that is, the times at which individual pulses appear at the output. Each pulse corresponds to a single photoelectron emitted at the photocathode.

\textbf{Here are the working principles of the technique:}
\begin{enumerate}
