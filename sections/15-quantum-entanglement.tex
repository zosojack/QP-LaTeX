...
The fondamental goal is to use entangled states to conduct optical test of Q.M., but we need to have single photons in order to conduct interference experiments and teleportation.\\\\
...
This means that making a measurement \textbf{doesn't reveal a pre-existing value of the observable}. The act of measurement make collapse the wawe function in a way that is not certain.\\\\
Suppose to prepare a photon as bit 1 in $\ket{V}$ state, a measurement of the diagonal component $\ket{D_+}$ with a beam splitter could return the value $+1$ (transmission) or $-1$ (reflection) with equal probability $50 \%$. Until the measurement is made it simply doesn't have a well defined value of the diagonal component. This reflect the \textbf{ortodox wiew of Q.M.}\\\\
...
\subsection{EPRB}
Now we discuss a semplified version of EPR proposed by Bohm:\\\\
...
In the "realist" point of wiew: the photon we measured was $\ket{H}$ from the moment it was emitted. Instead in the "ortodox" point of wiew: the measurement collapes the wawe function and produces istantaneously the state $\ket{H}$ for one photon and $\ket{V}$ for the other one. This contraddicts the relativistic principle: the information can't be faster than the light velocity.\\\\
\subsection{Bell experiment}
Allowed two polarizers to be indipendently adjusted (da capire). We have a polarizer A with $\vec{n}_A$ orientation and a polarizer B with $\vec{n}_B$ orientation. He proposed to compute the average of the product of the polarizations for a given pair of correlated photons and polarizers  orientation.\\\\
According to Q.M.
\[P(\vec{n}_A,\vec{n}_B) = -\vec{n}_A\cdot\vec{n}_B\]
Bell demostrate that this result is incompatible with any local hidenn variable theory. If we suppose to have a third polarizer $\vec{n}_C$, it is possible to derive the \textbf{Bell inequality}
...
if the inequeality is not satisfied, Q.M. is correct and hidden variable theory doesn't hold.\\\\
...
\begin{itemize}
    \item The ground state and the excitate state are $J=0$ states, which means that photon pairs have no net angular momentum.\\
    \item The parity of the energy levels implies polarization correlation.\\
\end{itemize}
The set up consists in a beam of $Ca$ atoms which is excite, this generates a pair of photons propagating in opposite directions, each one goes to a PM polarimeter (that measure the linear polarization), a filter $F_i$ (to select photon wawelengths) and then a PMT photomultiplier. Both the photomultipliers are connected to a same electronic system.
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{images_in_pdf/15-/Grafico_84.pdf}
\end{figure}

if we measure along $\vec{n}_A$ direction, this yelds to $\ket{V}$ if the polarization is parallel to $\vec{n}_A$ or $\ket{H}$ if the polarization is perpendiculare to $\vec{n}_A$. We can define the \textbf{correlation coefficient:}
...
In relistic local theory wiew the Bell inequality reads:
\[-2\leq S\leq 2\]
Instead according to Q.M. for particular orientations of $(\vec{n}_A,\vec{n}_A')$ and $(\vec{n}_B,\vec{n}_B')$, we can obtain:
\[S = \pm 2\sqrt{2}\]
this result contraddicts the Bell inequality called \textbf{CSCH inequality}. \\\\
