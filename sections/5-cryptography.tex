...
\calloutwarn{%
    This document follows the chronological order in which the topics were presented during the lectures; even if this section about cryptography is presented here in the first chapter, we strongly recommend to return to it after having read the third chapter. In particular, after the brief introduction to quantum computing given in Section~\ref{subsec:17-quantum-computing-overview}. 
}
...
\subsubsection{Polarized beam splitters}
...
\begin{equation}\label{eqn:5-generic-linear-polarization}
...
\[
\ket{D_+} \coloneqq
\frac{1}{\sqrt{2}}\left(\ket{H}+\ket{V}\right),
\qquad
\ket{D_-} \coloneqq
\frac{1}{\sqrt{2}}\left(\ket{H}-\ket{V}\right).
\]
...
\subsubsection{Eavesdropping and the cloning problem}
...
\end{enumerate}
The possible configurations are summarized in Table~\ref{tab:5-bb84}.
...
\begin{cases}
\hat{M_+} \ket{H} = 1 \cdot \ket{H}\\[0.5ex]
\hat{M_+} \ket{V} = -1 \cdot \ket{V}
\end{cases}
\hspace{1cm}
\begin{cases}
\hat{M_+} \ket{D_+} = 
\tfrac{1}{\sqrt{2}} \left(\ket{H} -\ket{V}\right) \equiv \ket{D_-}\\[0.5ex]
\hat{M_+} \ket{D_-} = 
...
A(+,0) \implies
\begin{cases}
E[P=50\%, +,0] \implies B[P=100\%, +,0] \implies P_{\text{tot}} = 50\% \\[2ex]
E[P=50\%, \times, (0,1)] \implies
\begin{cases}
B[P=50\%, +,0] \implies P_{\text{tot}} = 25\% \\[0.5ex]
...
N_\text{needed} = N \, [-\epsilon\, \log_2(\epsilon)-(1-\epsilon)\, \log_2(1-\epsilon)]
\end{equation}
