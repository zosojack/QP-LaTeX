...
Classical models describe light as a continuous train of electromagnetic waves, but this is only an approximation, since real light is emitted by atoms, with a finite lifetime. However, this lifetime is typically much shorter than the detector integration time $\tau_D$, which means that individual emission bursts cannot be resolved. As a consequence, one observes a well-defined average intensity that fluctuates on timescales longer than $\tau_D$. We can model classical light sources as a large number of atoms emitting independently. In a gas, atoms move and produce frequency shifts, while collisions interrupt the emission process and randomize the phase, leading to phase fluctuations.
Observe that:
\begin{itemize}
    \item A light source has a finite spectral width, meaning that its frequency is not perfectly constant. It also exhibits intensity noise, which implies that the amplitude of the light changes in time due to fluctuations in the number of emitting atoms and to random phase jumps in the field emitted by individual atoms. These features allow us to distinguish between classical and quantum light sources.
...
\end{itemize}
If we recall the Mach-Zehnder interferometer, the interference fringes are determined by the phase difference between the beams propagating along the two optical paths. All the discussion above assumes a stable source frequency; if the source has a finite spectral width $\Delta\omega$, the fringes are washed out because the contributions associated with different frequencies no longer interfere with full visibility, bright fringes with $\omega_1$ right overlap with dark fringes with $\omega_2$.

We can define \textbf{coherence} as the degree of correlation of the field, and it can be spatial or temporal. In the present discussion we are interested in temporal fluctuations, which are quantified by the \textbf{coherence time} $\tau_c$, defined as the duration over which the phase of the wave train remains correlated. In the same way, we can define the \textbf{coherence length} as
\[
L_c = c\,\tau_c.
\]
In a Mach-Zehnder interferometer, interference fringes can be observed only when the optical path difference is smaller than the coherence length $L_c$.

Suppose that:
 \begin{itemize}
     \item The light source is a \textbf{gas discharge lamp}, where the atoms are excited and emit radiation. The emission is determined by the spread in atomic velocities and by random emission events. Such a source is called a \textbf{chaotic light source}, like thermal cavities and filament lamps.
     \item We assume that \textbf{collision broadening dominates}, which causes elastic phase interruptions. Atoms emit trains of electromagnetic waves, but collisions shift the energy levels and interrupt the radiation emission. After a collision, emission resumes with the same characteristics as before, but the phase is now unrelated to the previous one. 
     Let us assume that the collision time is short, so that we can neglect the radiation emitted during the collision itself.
     \item Suppose to assume that the \textbf{polarization is fixed} and we can focus light in in order to obtain a well defined collimated beam far away from the source. 
 \end{itemize}
...
\begin{split}
E_{\text{tot}}(t) &= \sum_{i=1}^{N} E_i(t) \\
&= E_0 e^{-i\omega_0 t}\sum_{i=1}^{N} e^{i\phi_i(t)} \\
...
The total field therefore consists of a wave at frequency $\omega_0$ with a random amplitude $a(t)$ and a random phase.
...
We introduce the \textbf{Poynting vector} and suppose that the wave propagates along the $\vec{k}$ direction, along the $z$-axis:
...
\vec{S}&=\frac{1}{\mu_0}\,\vec{E}\times\vec{B}=\\
    ...
    \cos^2(\vec{k}\cdot\vec{r}-\omega t)=\\
    &=\varepsilon_0 c^2 E_0 B_0 \cos^2(\vec{k}\cdot\vec{r}-\omega t) \hat{k}
\end{split}
\end{equation}

If we take the cycle average over a period $T$ of the \ref{eqn:6-poynting-vector}:
...
In our case, with $E_\text{tot}(t)$ given by the \ref{eqn:6-total-field}:
\begin{equation}\label{eqn:6-chaotic-light-intensity}
    I=\varepsilon_0 c\,\langle \vec{E}^{\,2}\rangle_T
    =\varepsilon_0 c\cdot \frac{1}{2}E_0^2\cdot a^2(t)
\end{equation}
...
\]
From a physical point of view, \textbf{the coherence time represents the timescale over which the phase of the electromagnetic field remains correlated, i.e., how long the field “remembers” its phase.} Frequent collisions lead to rapid phase randomization and thus to a short coherence time.\\\\
In general, the \textbf{coherence time is determined by the spectral width and we can classify light with it}:
\begin{itemize}
    \item Perfect monochromatic source: $\Delta\omega=0$, which implies perfect phase correlation (ideal case, e.g. lasers), and therefore $\tau_c\to +\infty$.
    \item Thermal sources or white light: $\Delta\omega$ is broad, implying rapid phase fluctuations and therefore a very short coherence time.
\end{itemize}
Note that for a Doppler-broadened source the frequencies of the emitted radiation are distributed according to a Gaussian function. This originates from the thermal motion of atoms: each atom moves with a different velocity and, due to the Doppler effect, emits radiation at a slightly shifted frequency. Since the atomic velocities follow a Maxwell-Boltzmann distribution, the resulting frequency distribution is Gaussian. 
\begin{figure}[H]
    \centering
...
\begin{itemize}
    \item \textbf{Homogeneous broadening:} all atoms are affected in the same way. This is typically due to collision-induced phase interruptions and finite emission lifetime, and leads to a \textbf{Lorentzian line-shape}. It is fundamentally a temporal effect related to the finite coherence time of the emission process.
    \item \textbf{Inhomogeneous broadening:} different atoms are affected differently. In this case, each atom emits at a slightly different frequency due to its velocity (Doppler effect), leading to a \textbf{Gaussian line-shape}. This is a statistical effect arising from the distribution of atomic velocities in the ensemble.
\end{itemize}
...
\begin{itemize}
    ...
    \item The two beam splitters are identical and have the same transmission coefficient $T$ and reflection coefficient $R$ ($50{:}50$). These coefficients are also assumed to be independent of the frequency components of the light beam.
\end{itemize}
\begin{figure}[H]
    \centering
...
Along the path $z_1$ we have $\mathrm{Re}(E(t)) = E_1(t)$, while along the path $z_2$ we have $\mathrm{Re}(E(t)) = E_2(t)$. The field reaches the second beam splitter with delays
\[
t_1 = t - \frac{z_1}{c}, \qquad t_2 = t - \frac{z_2}{c}.
\]
Therefore,
\[
E_4(t) = R\cdot T \cdot E(t_1) + R\cdot T \cdot E(t_2).
\]
Averaging over a time window $T \gg \tau_c$, we obtain
\[
I_4(t) = \frac{1}{2}\varepsilon_0 c \cdot |E_4(t)|^2
= \frac{1}{2}\varepsilon_0 c \cdot |R|^2|T|^2
\left[\langle |E(t_1)|^2\rangle_T + \langle |E(t_2)|^2\rangle_T + 2\,\mathrm{Re}\langle E^*(t_1)E(t_2)\rangle_T\right].
\]
Notice that the first two terms are the intensities produced by each path separately, in the absence of interference, while the last term is the interference contribution and is determined by the \textbf{first-order correlation function}.
\[
...
Physically, $|g^{(1)}(\tau)|$ quantifies the ability of the field to produce interference: if $|g^{(1)}(\tau)| \approx 1$, the field is highly coherent and stable in phase, whereas if $|g^{(1)}(\tau)| \to 0$, the field has lost memory of its phase and no interference can be observed.

If we assume a collision-broadened light source, the electric field can be written as
\[
...
\]
The phase angles associated with different atoms are statistically independent and randomly distributed; therefore, all cross terms vanish upon averaging. As a result,
\[
\langle E^*(t)E(t+\tau)\rangle
...
After an atom undergoes a collision, its phase is randomized, effectively resetting its contribution to the field. Therefore, the single-atom correlation function is non-zero only if no collision occurs within the time interval $\tau$. In other words, it is proportional to the probability that the atom remains in free flight for a time longer than $\tau$.}
...
\]
where $\tau_0$ is the average time between collisions. The probability that no collision occurs in a time interval $\tau$ is then
\[
...
\]
which shows that the field loses phase memory exponentially in time. Finally, the degree of coherence becomes
\[
g^{(1)}(\tau)
...
Similarly, it can be shown that for a Doppler-broadened source:
\[
g^{(1)}(\tau) 
...
g^{(1)}(\tau)\to 0.
\]
\textbf{What happens if we consider a classical wave with stable amplitude and phase?}
\[
...
If we come back on M.Z. interferometer and average the intensity over a time interval $T$, we obtain:
\[
\begin{split}
    \langle I_4(t)\rangle_T &= \frac{1}{2}\varepsilon_0c\cdot |R|^2|T|^2\cdot
    \left[\langle |E(t_1)|^2\rangle_T+\langle |E(t_2)|^2\rangle_T+2\mathrm{Re}\langle E^*(t_1)E(t_2)\rangle_T\right] \\
    &= \frac{1}{2}\varepsilon_0c\cdot |R|^2|T|^2\cdot \langle |E(t)|^2\rangle\cdot
    \left[2+2\mathrm{Re}\frac{\langle E^*(t_1)E(t_2)\rangle}{\langle |E(t)|^2\rangle}\right] \hspace{1cm} |E(t_1)| = |E(t_2)| \hspace{0.2cm}\text{through a B.S. 50:50}\\
    \langle I_4(t)\rangle_T &= 2\cdot |R|^2|T|^2\cdot \langle I(t)\rangle\cdot
    \left[1+\mathrm{Re}\bigl(g^{(1)}(\tau)\bigr)\right]
\end{split}
\]
The output intensity is therefore proportional to $\langle I(t)\rangle$ and modulated by the factor $1+\mathrm{Re}(g^{(1)}(\tau))$.\\
The fringe visibility is given by
\[
V=|g^{(1)}(\tau)|=\frac{I^{\max}-I^{\min}}{I^{\max}+I^{\min}}.
\]
For chaotic light with Lorentzian broadening,
\[
g^{(1)}(\tau)=e^{-i\omega_0\tau-\frac{|\tau|}{\tau_c}},
\]
so that
\[
\langle I_4(t)\rangle=2\cdot |R|^2|T|^2\cdot \langle I(t)\rangle\cdot
\left[1+e^{-\frac{|\tau|}{\tau_c}}\cos(\omega_0\tau)\right].
\]
...
\input{tables/6-/type-of-light.tex}