...
\[
E(x,t) = E_0 \, \cos(kx-\omega_0 t + \phi),
\]
...
\[
\Phi = \frac{P}{\hbar \omega}, \qquad \overline{n} = \Phi \, \frac{L}{c}.
\]
...
&= \frac{N!}{n!(N-n)!}\, \left(\frac{\overline{n}}{N}\right)^n \, \left(1-\frac{\overline{n}}{N}\right)^{N-n}.
\end{split}
\]
...
P(n) = \frac{1}{n!}\, (\overline{n})^n\, e^{-\overline{n}}, \qquad n=0,1,2,\dots
\end{equation}
...
\[
P_{\omega}(n)= \frac{1}{\overline{n}+1}\, \left(\frac{\overline{n}}{\overline{n}+1}\right)^n,
\qquad
...
\begin{split}
    a&.
    \begin{cases}
    a_1.\ n \text{ events} \hspace{0.55cm} &t \to t+\tau\\[0.5ex]
    a_2.\ 0 \text{ events}              &t+\tau \to t+\tau+\d\tau
    \end{cases}
    \\[1ex]
    b&.
    \begin{cases}
        b_1.\ n-1 \text{ events} &t \to t+\tau\\[0.5ex]
...
\begin{split}
    P_n(t,\tau+d\tau) 
    &= P(a) + P(b) = \\[0.5ex]
    &= P_n(t,\tau)\,[1-p(t+\tau)\,d\tau] + 
    P_{n-1}(t,\tau)\,p(t+\tau)\,\d\tau = \\[0.5ex]
...
\begin{split}
    ...
    \frac{P_n(t,\tau+d\tau) - P_n(t,\tau)}{\d\tau} =\\[0.5ex]
...
\int_0^T \d\tau\, C\,e^{C\tau}\,P_{n-1}(\tau); 
\tag*{n.b.: $P_n(0)=0$}\\[0.5ex]
