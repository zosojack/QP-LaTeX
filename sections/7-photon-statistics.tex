...
Intensity fluctuations can be regarded as a fluctuations in the number of photons emitted by a source, because o f that we can use photon number distribution function in order to classify light. In particular we have three type of photon statistics:
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item $\Delta n \approx \sqrt{\overline{n}}$
    \end{itemize}
...
\end{table}
The first two types of statistics are consistent with classical theory of light, while the third one leads to quantum description. This happens because light is very sensitive to losses and we need an efficient detective system that it has been observed only recently.

\subsection{Poissonian photon statistics}
...
\textbf{Is the stream of photons emitted at regular time intervals?}\\
...
\textbf{Derivation:}\\
Assume a light beam with constant optical power $P$ and photon flux. Let $\overline{n}$ be the average number of photons within a beam segment of length $L$:
\[
...
\begin{itemize}
    ...
    \[
    \sigma^2 = (\Delta n)^2 = \sum_{n=0}^{\infty} (n-\overline{n})^2 P(n) = \overline{n}.
    \]
    \item Therefore, the standard deviation is
    \[
...
\end{itemize}
\subsection{Super Poissonian light}
A black body can be modeled as an enclosed cavity at temperature $T$ filled with electromagnetic radiation. In this case, the field consists of a continuous spectrum of oscillating modes with energy density $\rho(\omega,T)$, where $\omega$ is the angular frequency of a given mode. Each mode behaves as a quantized harmonic oscillator with energy
\[
E_n=\hbar\omega\left(n+\frac{1}{2}\right).
\]
This leads to the Planck distribution:
\[
\rho(\omega,T)\cdot d\omega = \frac{\hbar \omega^3}{\pi^2 c^3}\cdot \frac{1}{e^{\beta \hbar \omega}-1}\cdot d\omega.
\]
...
\[
\begin{split}
    P_{\omega}(n) &= \frac{e^{-\beta E_n}}{\sum_{n=0}^{\infty} e^{-\beta E_n}}\\
    &= \frac{e^{-\beta \cdot \hbar \omega \cdot n}}{\sum_{n=0}^{\infty} e^{-\beta \cdot \hbar \omega \cdot n}}\\
    &= \frac{x^n}{\sum x^n}, \qquad x=e^{-\beta\hbar\omega}\\
    &= e^{-\beta\cdot \hbar \omega \cdot n}\cdot \left(1-e^{-\beta\cdot \hbar \omega}\right),
\end{split}
\]
since
\[
...
\]
The first term comes from Poissonian shot noise, which reflects the particle nature of light, while the second term represents classical power fluctuations. Here
\begin{itemize}
    \item $W(T)$ is the count rate within a detection time $T$:
    \[
    W(T) = \int_t^{t+T} \eta \cdot \Phi(t')\,dt',
    \]
...
\]
In our case, however, $\Phi \neq \text{constant}$ and two different regimes can be identified:
\begin{enumerate}
    \item The intensity fluctuations are relevant when
    \[
    T \leq \tau_c \implies \langle \delta W(T)^2\rangle \neq 0.
    \]
    In this regime, the detection time is short enough that the detector can resolve the temporal fluctuations of the light field. Since the phase and intensity remain correlated over a time of order $\tau_c$, the measured signal reflects not only the Poissonian shot noise (due to the particle nature of light), but also the classical intensity fluctuations of the source. Physically, this corresponds to the fact that thermal light exhibits photon bunching, so photons tend to arrive in correlated groups rather than independently.
    \item The Poissonian behavior is approached when
    \[
    ...
    \]
    The measured statistics therefore approaches the Poissonian limit, where fluctuations are dominated by shot noise. Physically, this does not mean that the light itself becomes Poissonian, but rather that the measurement process averages over the fluctuations, making the detected signal appear more regular.
\end{enumerate}
...
Suppose to have a light beam where every time interval between the photons is the same $\Delta x = c\cdot \Delta t$. In a time $T$ the photocount will be:
\[N = int\biggr(\eta \cdot \frac{T}{\Delta t}\biggr)\]
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{images_in_pdf/7-/Grafico_29.pdf}
\end{figure}

This kind of photon stream is called \textbf{Photon Number states}. Observe that the time intervals $\Delta t$ are not exactly the same, but more "regular" than the random intervals pertaining to beams having poissonian statistics.\\
...
\end{itemize}
This leads us to study photodetection.
\subsubsection{Photodetection}
\textbf{How are the electric field and light intensity converted into an experimental signal?}\\
...
For example, consider a red laser with wavelength $\lambda = 633$ nm and power $P \approx 1$ nW. The photon flux impinging on the detector is
\[
...
We can define:
\begin{itemize}
    \item  the \textbf{quantum efficiency} of the detector as
\[
\eta = \frac{\text{Number of photocounts}}{\text{Number of incident photons}}.
\]
\item  During a time interval $T$, the \textbf{average number of counts} recorded is
\[
M(T) = \eta \cdot \Phi \cdot T = \eta \cdot \frac{P}{\hbar \omega} \cdot T,
\]
\item The corresponding \textbf{average count rate} is
\[
R = \frac{M(T)}{T} = \eta \cdot \frac{P}{\hbar \omega} \quad [\text{counts/s}].
\]
\end{itemize}
If the detector has a \textbf{dead time}, which is a recovery period after each detection event, this limits the maximum count rate. For instance, a dead time of $1$ ns corresponds to a maximum rate of $R \approx 10^9$ counts/s.\\\\
It is also important to observe that $R$ and $\Phi$ are average quantities, while the actual photon flux exhibits statistical fluctuations due to the discrete nature of light. \textbf{These fluctuations of the measured count rate $R$ can be used to study the photon statistics of the light beam.}\\
...
\begin{itemize}
    \item Each photon is registered with probability $\eta$, so the photocounts follow a Bernoulli process for each incident photon.
...
\end{itemize}
As a result, the observed photocounts reflect both the statistical properties of the light beam and the stochastic nature of the detection process. Therefore, when analyzing photon statistics, it is essential to distinguish between fluctuations due to the light source itself and those introduced by the detector.
...
Suppose that a light beam impinges on a photocathode, so that a single electron is emitted by the photoelectric effect and then amplified by an electron multiplier. This process gives rise to an output pulse and, consequently, to a count.\\\\
In this semiclassical model, atoms and the detector are treated quantum mechanically, while the electromagnetic field is treated classically. The number of photocounts produced in an integration time $T$ is therefore a measure of the optical intensity. We consider a large number of successive measurements, each performed over the same integration time, in order to build the probability distribution $P_n(T)$ for observing $n$ photocounts during the interval $T$.\\\\
...
\[\begin{split}
    &a)\begin{cases}
    \text{$n$ events} \hspace{1cm} t \to t+\tau\\
    \text{$0$ events} \hspace{1cm} t+\tau \to t+\tau+d\tau
\end{cases}\\
&b)\begin{cases}
    \text{$n-1$ events} \hspace{1cm} t \to t+\tau\\
    \text{$1$ event} \hspace{1cm} t+\tau \to t+\tau+d\tau
\end{cases}
\end{split}\]
...
\]
\textbf{Thus, we recover the Poisson distribution, which means that the photocount statistics are the same as those of particle emission in radioactive decay.\\\\
